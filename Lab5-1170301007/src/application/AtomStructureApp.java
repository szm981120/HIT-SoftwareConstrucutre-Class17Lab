...
import iostrategy.AtomStructureIoStrategy;
...
/**
 * AtomStructureAPP provides a static method for client to call in Main.java. This static method is
 * the mainly function method for atom structure applications.
 * 
 * @author Shen
 *
 */
public class AtomStructureApp {
  ...
  public static void application() {
    CircularOrbitApis<Nucleus, PhysicalObject> apis =
    ...
    AtomStructureIoContext fileReaderContext =
    ...
    while (true) {
      ...
      switch (choose) {
        case 1: // Read from file to generate a atomic structure
          ...
          try {
            switch (choose1) {
              case 1:
                atomStructureLogger
                    .info("Read from input/AtomicStructure.txt. Restore present state.");
                file = new File("input/AtomicStructure.txt");
                // atomStructure.readFromFile(file);
                fileReaderContext.readFromFile(atomStructure, file);
              ...
              case 2:
                atomStructureLogger
                    .info("Read from input/AtomicStructure_Medium.txt. Restore present state.");
                file = new File("input/AtomicStructure_Medium.txt");
                // atomStructure.readFromFile(file);
                fileReaderContext.readFromFile(atomStructure, file);
                caretaker.addMemento(atomStructure.save());
                atomStructureLogger.info(
                    "Success: Read from input/AtomicStructure_Medium.txt. Restore present state.");
                break;
              case 3:
        ...
        case 4:// Add an object to a track
          ...
          atomStructureLogger
              .info("Success: Add an electron to track with " + addElectronRadius + " radius.");
          break;
        ...
        case 6: // Delete an object from a track
          ...
          try {
            ...
            atomStructureLogger.log(Level.SEVERE,
                e.getMessage() + " Fail track: " + deleteElectronRadius + ".", e);
            break;
          }
        ...
        case 7: // Calculate the information entropy of the system
          ...
          atomStructureLogger.log(Level.INFO,
              "Success: Calculate the information entropy: " + entropy + ".");
          break;
        case 8: // Transition
          ...
          try {
            ...
            atomStructureLogger.log(Level.SEVERE,
                e.getMessage() + " Fail track: " + sourceRadius + ".", e);
            break;
          }
          atomStructure.addPhysicalObjectToTrack(electronFactory.produce(),
              trackFactory.produce((double) targetRadius));
          caretaker.addMemento(atomStructure.save());
          atomStructureLogger.log(Level.INFO, "Success: Transition an electron from track "
              + sourceRadius + " to track" + targetRadius + ". Restore present state.");
          break;
        case 9: // Restore transition
          ...
          atomStructureLogger.log(Level.INFO,
              "Success: Roll back " + countdownVersion + " transition versions.");
          break;
        case 10: // Log search
          apis.logSearch(new File("log/atomStructureLog.log"));
          break;
        case 11: // Write the current atom structure to file
          atomStructureLogger.info("Write the current atom structure to file.");
          System.out.println("Please input the path of the written file.");
          String writePath = in.nextLine();
          File writeFile = new File(writePath);
          try {
            // atomStructure.writeToFile(writeFile);
            fileReaderContext.writeToFile(atomStructure, writeFile);
          } catch (IOException e) {
            e.printStackTrace();
          }
          atomStructureLogger
              .info("Success: Write the current atom structure to " + writePath + ".");
          break;
        default:
          in.close();
  ...
  private static void menu() {
    ...
    System.out.println("11. Write the current atom structure to file.");
  }
