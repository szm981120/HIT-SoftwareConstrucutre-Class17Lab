...
/**
 * Caretaker works in Memento method as a state histories keeper in atom structure. It keeps state
 * histories.
 * 
 * <p>Attention: memento only keeps states in transition changes. Any other change not in transition
 * will not be kept in memento. For example, supposed that an electron is deleted first and then a
 * transition happens, if you roll back one step from now, it will go back to the state before the
 * electron is deleted.
 * 
 * @author Shen
 *
