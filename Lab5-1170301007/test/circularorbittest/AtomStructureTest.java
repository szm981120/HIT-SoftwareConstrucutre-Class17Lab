...
import atomtransitionmement.Caretaker;
...
public class AtomStructureTest {
  ...
  /*
   * Test strategy All tests below are about tests in atom structure.
   * 
   * @Before readFromFile Before any test, atomStructure should be initialized from
   * AtomicStructure.txt. The test is all about atom structure from AtomicStructure.txt.
   * testMementoMethod This tests Memento method in transition action. It does two transition to
   * atom structure and saves these two states. And it restores these states to see if the
   * transition can be undone. testAddTrack This tests addTrack. Add a track to atom structure to
   * see the change before and after adding. Call getTracks to see if the test track is added.
   * testDeleteTrack This tests deleteTrack. Delete a track in atom structure to see the change
   * before and after deleting. Call getTracks to see if the track in test position is missed.
   * testAddCentralObject This tests addCentralObject. Add a central object in atom structure to see
   * the change before and after adding. Call getCentralObject to see if the central object is the
   * test one after adding. testAddPhysicalObjectToTrack This tests addPhysicalObjectToTrack. Add a
   * physical object to track to see the change before and after adding. Call getObjectsInTrack to
   * see if the number of objects in corresponding track changes after adding.
   * testDeletePhysicalObjectFromTrack This tests deletePhysicalObjectFromTrack. Delete a physical
   * object from track to see the change before and after deleting. Call getObjectsInTrack to see if
   * the number of objects in corresponding track changes after deleting. testResetObjectsAndTrack
   * This tests resetObjectsAndTrack. Call resetObjectsAndTrack and then see if the tracks and
   * objectInTracks is empty. testGetCentralObject This tests getCentralObject. See if the central
   * object fits the data in file. testGetTracks This tests getTracks. See if the tracks fits the
   * data in file. testGetObjectsInTrack This tests getObjectInTrack. See if the objects in tracks
   * fits the data in file. testConstructSocialNetworkCircle Test that atom structure shouldn't have
   * constructSocialNetworkCircle method. testAddRelationshipBetweenCentralAndPhysical Test that
   * atom structure shouldn't have addRelationshipBetweenCentralAndPhysical method.
   * testAddRelationshipBetweenPhysicalAndPhysical Test that atom structure shouldn't have
   * addRelationshipBetweenPhysicalAndPhysical method.
   * testDeleteRelationshipBetweenPhysicalAndPhysical Test that atom structure shouldn't have
   ...
   * testGetRelationBetweenObjects Test that atom structure shouldn't have getRelationBetweenObjects
   * method.
   */
  ...
  public void readFromFile() {
    ...
    AtomStructureIoContext asContext = new AtomStructureIoContext(new AtomStructureFileReader());
    try {
      // atomStructure.readFromFile(file);
      asContext.readFromFile(atomStructure, file);
  ...
  public void testAddPhysicalObjectToTrack() {
    assertEquals("expected 8", 8,
        atomStructure.getObjectsInTrack().get(atomStructure.getTracks().get(1)).size());
    atomStructure.addPhysicalObjectToTrack(electronFactory.produce(),
        atomStructure.getTracks().get(1));
    assertEquals("expected 9", 9,
        atomStructure.getObjectsInTrack().get(atomStructure.getTracks().get(1)).size());
  }
  ...
  public void testConstructSocialNetworkCircle() {
    atomStructure.constructSocialNetworkCircle(null);
  }

  @Test(expected = AssertionError.class)
  public void testAddRelationshipBetweenCentralAndPhysical() {
    atomStructure.addRelationshipBetweenCentralAndPhysical(electronFactory.produce(), 0.5);
  }

  @Test(expected = AssertionError.class)
  public void testAddRelationshipBetweenPhysicalAndPhysical() {
    atomStructure.addRelationshipBetweenPhysicalAndPhysical(electronFactory.produce(),
        electronFactory.produce(), 0.5);
  }
